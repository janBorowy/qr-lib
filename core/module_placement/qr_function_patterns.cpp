...
void draw_finder_pattern(MutableQrCode& qr, int x, int y) {
    draw_square(qr, x, y, 7, BLACK[0]);
    draw_square(qr, x + 1, y + 1, 5, WHITE[0]);
    draw_square(qr, x + 2, y + 2, 3, BLACK[0]);
    qr[x + 3, y + 3] = BLACK[0];
}

void draw_alignment_pattern(MutableQrCode& qr, int center_x, int center_y) {
    draw_square(qr, center_x - 2, center_y - 2, 5, BLACK[0]);
    draw_square(qr, center_x - 1, center_y - 1, 3, WHITE[0]);
...
void draw_timing_patterns(MutableQrCode& img, size_t size) {
    for (int x = 8; x < size - 8; x++) {
        img[x, 6] = x % 2 == 0 ? BLACK[0] : WHITE[0];
    }
    for (int y = 8; y < size - 8; y++) {
        img[6, y] = y % 2 == 0 ? BLACK[0] : WHITE[0];
    }
}
...
void draw_function_patterns(MutableQrCode& img, int version) {
    ...
    img.draw_rect(WHITE[0], 0, 7, 7, 7);
    img.draw_rect(WHITE[0], 7, 0, 7, 7);

    draw_finder_pattern(img, size - 7, 0);
    img.draw_rect(WHITE[0], size - 8, 0, size - 8, 7);
    img.draw_rect(WHITE[0], size - 1, 7, size - 8, 7);

    draw_finder_pattern(img, 0, size - 7);
    img.draw_rect(WHITE[0], 0, size - 8, 7, size - 8);
    img.draw_rect(WHITE[0], 7, size - 8, 7, size - 1);
